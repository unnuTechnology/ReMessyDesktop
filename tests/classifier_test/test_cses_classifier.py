...
@pytest.mark.parametrize("now, expected", [
    (datetime(2026, 3, 2, 7, 30, 0), ClassificationResult.UNKNOWN),  # 所有课开始前
    ...
    (datetime(2026, 3, 3, 10, 30, 0), ClassificationResult.UNKNOWN),  # 单周，放学后
    (datetime(2026, 3, 4, 8, 30, 0), ClassificationResult.UNKNOWN),  # 找不到课表
])
def test_cses_classifier(now, expected):
    result = cses.cses_classifier(Path(""), example_config, now_tm=now)  # ty: ignore  # pyright: ignore
