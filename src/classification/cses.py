...
def cses_classifier(
    _: Path, config: Config, now_tm: datetime.datetime | None = None
) -> str | ClassificationResult:
    log.debug(
        f'正在从 CSES 课表 ({config.classification.cses_classifier.cses_path}) 中获取今日课程'
    )
    if now_tm is None:
    ...
    try:
        today_classes = schedule.today_schedule(
            start_day=parse(
                config.classification.cses_classifier.start_day, yearfirst=True
            ).date(),
            day=now_tm.date(),
        ).classes
    ...
    times_subject = [
        ((lesson.start_time, lesson.end_time), lesson.subject)
    ...
    for i, ((start, end), subject) in enumerate(times_subject):
        try:
            if start <= now <= end:  # 上课时
                log.debug(
                    f'当前时间 ({now}) 在 {subject}({start}-{end}) 之间，返回课程 ({subject})'
                )
                return subject
            if end <= now < times_subject[i + 1][0][0]:  # 上节课下课，下一个课程开始之前（下课）
                log.debug(
                    f'当前时间 ({now}) 在 课间({end}-{times_subject[i+1][0][0]}) 之间，返回课程 ({subject})'
                )
